 ...
 * 
 *         Basado en
 *         https://github.com/ttddyy/datasource-proxy-examples/blob/master/hibernate-example/src/main/java/net/ttddyy/dsproxy/example/Application.java
 *
 */
...
//            String gestorUtilizado="MySQL";
			...
			// org.hibernate.dialect.MySQLDialect versión 5
			...
			MetadataSources metadataSources = new MetadataSources(standardRegistry).addPackage("model")
					.addAnnotatedClass(hibernate.util.Cliente.class)
					.addAnnotatedClass(hibernate.util.Pedido.class);
	...
	private static DataSource getDataSource() {
		...
		return ProxyDataSourceBuilder.create(getMySQLDataSource()) // ojo
				.name("ProxyDataSource").countQuery().multiline().listener(listener)
	...
	private static DataSource getMySQLDataSource() {
		...
		ds.setURL("jdbc:mysql://10.11.1.171:3306/Cliente");
		ds.setUser("root");
		ds.setPassword("root");
		try {
	...
	public static String getEstadoObjeto(Object objeto, Session session) {
		...
		try {
			...
			mensajeExcepcion = "Se ha producido una excepción: " + e.getMessage();
			// e.printStackTrace();
		}
